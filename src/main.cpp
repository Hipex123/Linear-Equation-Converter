...
#include "Libs/Includes/evaluator.hpp"
#include "Libs/Includes/linearEquationConverter.hpp"
...
private:
    ...
    void createDescriptorPool()
    {
        ...
        poolInfo.maxSets = static_cast<uint32_t>(textureToCreate + MAX_FRAMES_IN_FLIGHT);
    ...
    void createDescriptorSets()
    {
        ...
        allocInfo.descriptorSetCount = static_cast<uint32_t>(textureToCreate + MAX_FRAMES_IN_FLIGHT);
    ...
    void updateUniformBuffer(uint32_t currentImage)
    {
        ...
        glm::mat4 x = glm::rotate(glm::mat4(1.0f), glm::radians(180.0f), glm::vec3(0, 0, 1));
        ubo.model = glm::translate(x, modelPosition);
        ubo.view = glm::lookAt(camPosition, camLookCords, glm::vec3(0.0f, 1.0f, 0.0f));
            ...
            if (mouseXposNorm > buttonNormCoords[i][0].x && mouseYposNorm < buttonNormCoords[i][0].y && mouseYposNorm > buttonNormCoords[i][1].y && mouseXposNorm < buttonNormCoords[i][1].x)
            {
                ...
                case 6:
                    if ((firstFuncType != 0 && secondFuncType != 0) && firstFuncType != secondFuncType && !inputBoxes[0].empty() && !inputBoxes[1].empty())
                    {
                        ...
                        auto result = linearEquationConverter::convertLinearEquation(firstFuncType, secondFuncType, globalConverts);
